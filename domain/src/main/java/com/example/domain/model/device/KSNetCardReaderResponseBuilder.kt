    ...
    fun receiveData(data: ByteArray): PaymentProcessStatus {
        if (receive(data) == 1) {
            ...
            when (getCommandID(resultData, resultDataLength)) {
                0xD0.toByte() -> {
                    isFirstPayment = true
                    return PaymentProcessStatus.DeviceCommunication.InsertIC
...
//                                ProcessAfterSerialCommunicate.RequestFallback.name, cardType))
                        ...
                        return PaymentProcessStatus.DeviceCommunication.FallBack(
...
//                                    ProcessAfterSerialCommunicate.RequestCardNumber.name))
                            ...
                            return PaymentProcessStatus.DeviceCommunication.ReadingIC
